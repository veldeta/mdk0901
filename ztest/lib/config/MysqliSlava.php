...
class MysqliSlava 
{
    ...
    function __construct(){
        self::$db = require_once __DIR__ . '../db.php';
        $this->mysqli = new mysqli(self::$db['host'], self::$db['user'], self::$db['password'], self::$db['dbname']);
    }

    public function myOpen(){
        
        if($this->mysqli->error){
            throw new MysqliError("Не Удалось подключиться к базе данных.", $this->mysqli->error, $this->mysqli->errno);
        }
        return $this->mysqli;
    }
